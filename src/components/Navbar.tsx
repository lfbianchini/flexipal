...
const Navbar = () => {
  ...
  return (
    <nav ref={navRef} className="w-full bg-white shadow-card rounded-b-xl px-4 py-2 md:py-3 md:px-8 flex items-center z-30 relative">
      ...
      <button
        className="ml-auto inline-flex md:hidden p-2 rounded-lg hover:bg-gray-100 transition"
        onClick={() => setMobileOpen((val) => !val)}
        aria-label="Toggle menu"
      >
        <Menu size={22} className="text-usfgreen" />
      </button>
      {/* Center all nav items horizontally on desktop */}
      <div className="hidden md:flex flex-1 items-center justify-center space-x-6">
        {visibleNavItems.map(({ name, path, icon: Icon }) => (
          <Link
            key={name}
            to={path}
            className={`flex items-center px-3 py-1.5 rounded-lg text-sm font-medium gap-2 transition hover:bg-gray-100 hover:text-usfgreen ${
              pathname === path
      ...
      {mobileOpen && (
        <div className="absolute top-full left-0 w-full bg-white border-t border-graydivider shadow-card z-20 flex flex-col items-stretch animate-fade-in md:hidden">
          {visibleNavItems.map(({ name, path, icon: Icon }) => (
            <Link
              key={name}
              to={path}
